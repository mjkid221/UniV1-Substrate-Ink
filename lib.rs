...
mod amm {   
    ...
    #[ink(event)]
    pub struct SupplyLiquidity {
        #[ink(topic)]
        sender: Option<AccountId>,
        share: Balance,
        token_amount_1: Balance,
        token_amount_2: Balance
    }
    #[ink(event)]
    ...
    impl Amm {
        ...
        pub fn provide(
            ...
            let currentShare = self.shares.get(&caller).unwrap_or_default();
            self.shares.insert(caller, &(currentShare + share));

            self.env().emit_event(SupplyLiquidity {
                sender: Some(caller),
                share: share,
                token_amount_1: _amountToken1,
                token_amount_2: _amountToken2
            });
            Ok(share)
        }
        ...
        pub fn withdraw(&mut self, _share: Balance) -> Result<(Balance, Balance), Error> {
            ...
            let currentShare = self.shares.get(&caller).unwrap();
            self.shares.insert(caller, &(currentShare - _share));
            self.totalShares -= _share;

            self.totalToken1 -= amountToken1;
            self.totalToken2 -= amountToken2;

            let currentToken1Balance = self.token1Balance.get(&caller).unwrap_or_default();
            self.token1Balance.insert(caller, &(currentToken1Balance + amountToken1));

            let currentToken2Balance = self.token2Balance.get(&caller).unwrap_or_default();
            self.token2Balance.insert(caller, &(currentToken2Balance + amountToken2));
            
            self.env().emit_event(WithdrawLiquidity {
                sender: Some(caller),
                share: _share,
                token_amount_1: amountToken1,
                token_amount_2: amountToken2
            });
            Ok((amountToken1, amountToken2))
        }
        ...
        pub fn swapToken1GivenToken1(
            ...
            let currentToken1Balance = self.token1Balance.get(&caller).unwrap();
            self.token1Balance.insert(caller, &(currentToken1Balance - _amountToken1));
            
            self.totalToken1 += _amountToken1;
            self.totalToken2 -= amountToken2;
            
            let currentToken2Balance = self.token2Balance.get(&caller).unwrap();
            self.token2Balance.insert(caller, &(currentToken2Balance + amountToken2));

            self.env().emit_event(Swap {
                sender: Some(caller),
                input_amount: _amountToken1,
                output_amount: amountToken2
            });
            Ok(amountToken2)
        }
        ...
        pub fn swapToken1GivenToken2(
            ...
            let currentToken2Balance = self.token2Balance.get(&caller).unwrap();
            self.token2Balance.insert(caller, &(currentToken2Balance + _amountToken2));

            self.env().emit_event(Swap {
                sender: Some(caller),
                input_amount: amountToken1,
                output_amount: _amountToken2
            });
            Ok(amountToken1)
        }
        // -----------------------------Token2=>Token1-----------------------------------
        ...
        pub fn swapToken2GivenToken2(
            ...
            self.env().emit_event(Swap {
                sender: Some(caller),
                input_amount: _amountToken2,
                output_amount: amountToken1
            });
            Ok(amountToken1)
        }
        ...
        pub fn swapToken2GivenToken1(
            ...
            self.env().emit_event(Swap {
                sender: Some(caller),
                input_amount: amountToken2,
                output_amount: _amountToken1
            });
            Ok(amountToken2)
        }
    ...
    mod tests {
        ...
        #[ink::test]
        fn should_swap_token1_to_token2_given_token1() {
            let mut contract = Amm::new(0);
            contract.faucet(100, 200);
        ...
        #[ink::test]
        fn should_swap_token1_to_token2_given_token2(){
            let mut contract = Amm::new(0);
            contract.faucet(100, 200);
            let share = contract.provide(50, 100).unwrap();
            let amountToken1 = contract.swapToken1GivenToken2(25, 50).unwrap();
            assert_eq!(amountToken1, 16);
            assert_eq!(contract.getMyHoldings(), (34, 125, share));
            assert_eq!(contract.getPoolDetails(), (66, 75, share, 0));
        }
        #[ink::test]
        fn should_swap_token2_to_token1_given_token2(){
            let mut contract = Amm::new(0);
            contract.faucet(200, 200);
            let share = contract.provide(100, 50).unwrap();
            let amountToken1 = contract.swapToken2GivenToken2(50, 50).unwrap();
            assert_eq!(amountToken1, 50);
            assert_eq!(contract.getMyHoldings(), (150, 100, share));
            assert_eq!(contract.getPoolDetails(), (50, 100 , share, 0))
        }
        #[ink::test]
        fn should_swap_token2_to_token1_given_token1(){
            let mut contract = Amm::new(0);
            contract.faucet(200, 100);
            let share = contract.provide(100, 50).unwrap();
            let amountToken2 = contract.swapToken2GivenToken1(25, 50).unwrap();
            assert_eq!(amountToken2, 16);
            assert_eq!(contract.getMyHoldings(), (125, 34, share));
            assert_eq!(contract.getPoolDetails(), (75, 66, share, 0));
        }
